import jwt
from datetime import datetime
from datetime import timedelta
import urllib.request
import urllib.parse
import requests
import json
from pathlib import Path


home = str(Path.home())

#reading the file ~/privkey.pem 
privkeyfile = home + '/privkey.pem'
with open(privkeyfile, 'r') as myfile:
    priv_key=myfile.read()

#the x5c certificate value of Krijn B.V., necessary to create the jwt header used in client_assertion 
x5c_value = [""]

#the id of the client making the request at Scheme Owner
client_id = "EU.EORI.NL219048113"

#creating the client_assertion jwt token
encoded_jwt = jwt.encode(
    {
  "iss": client_id,
  "sub": client_id,
  "aud": "EU.EORI.NL000000000",
  "jti": "378a47c4-2822-4ca5-a49a-7e5a1cc7ea59",
  "exp": datetime.utcnow() + timedelta(seconds=30),
  "iat": datetime.utcnow(),
  }, priv_key, algorithm='RS256', headers={ 'x5c': x5c_value})

print("Access token?")

#requests an access token at Scheme Owner using the client assertion created above
r = requests.post("https://scheme.isharetest.net/connect/token", data =
                  { "grant_type": "client_credentials",
                    "scope": "iSHARE",
                    "client_id": client_id,
                    "client_assertion_type": "urn:ietf:params:oauth:client-assertion-type:jwt-bearer",
                    "client_assertion": encoded_jwt
                    })

#prints if the access token is received
if r.status_code == 200:
    print("Access token obtained.")

#access token into json object
json_r = r.json()
access_token = "Bearer " +json_r["access_token"]

#request the party info from the Scheme Owner, using the previously received access_token
s = requests.get("https://scheme.isharetest.net/trusted_list", headers =
                 { "Authorization": access_token}, params = { "name": "Krijn"})

#prints the decoded info of the trusted_list in the Scheme Owner response
json_s = s.json()
cap_token = json_s["trusted_list_token"]
dec_cap_token = jwt.decode(cap_token, verify=False)
print(json.dumps(dec_cap_token, indent = 4, sort_keys=False))
