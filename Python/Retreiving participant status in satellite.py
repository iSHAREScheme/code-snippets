...
satelliteID = "EU.EORI.EMIL"
satelliteURL = "https://emilsatellite-mw.isharesatellite.eu"
...
POST_req = requests.post(satelliteURL + "/connect/token", headers=POST_header, data=POST_parameters)
print(POST_req)

# Retrieving the access token 
access_token = POST_req.json()["access_token"]

# Identifier of external party
partyID = "EU.EORI.EMILSC"

# Party name of external party
partyName = "EmilServiceConsumer"

# HTTP GET header
GET_header = {"Authorization":"Bearer " + access_token}

# HTTP GET parameters. 
# For a more refined (only contain some specific info) response token, add optional parameters (documented in the iSHARE dev portal)
GET_parameters = {"name": partyName, 
                  "eori": partyID}

#  Making a get request to obtain the status of a different party
GET_req = requests.get(satelliteURL + "/parties", headers=GET_header, params=GET_parameters)
print(GET_req)

# Retrieving the response token
response_token_encoded = GET_req.json()["parties_token"]

# Decoding the response token
response_token = jwt.decode(response_token_encoded, options = {"verify_signature": False})

# Find the party's status in the dataspace in the response token
print(response_token["parties_info"]["data"][0]["adherence"])
